...

{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        // getSupportActionBar().setSubtitle("Page2");
    ...
    private void calculate(String input)
    {
        ...
        try
        {
            ...
            if (equalClicked)
            {
                temp = input + lastExpression; // temp is the expression to be calculated
                String lastInput = temp.charAt(temp.length() - 1) + "";
                if (lastInput.equals("+") || lastInput.equals("-") || lastInput.equals("x") || lastInput.equals("\u00F7") || lastInput.equals("%"))
                {
                    temp = temp + textViewInputNumbers.getText().toString(); // if the last input is an operand, then add the current input to the expression and calculate
        ...
        } catch (Exception e)
        {
            ...
            textViewInputNumbers.setText("Error");
            //disable the operands buttons
            ...
            buttonParentheses.setEnabled(false);

            // if clear button is clicked, enable the operands buttons
            buttonClear.setOnClickListener(new View.OnClickListener()
            {
                @Override // this override means that the method is overriding the method in the parent class
                // meaning that the method is already defined in the parent class
                public void onClick(View v)
                {
                    ...
                    buttonParentheses.setEnabled(true);
        ...
        //IF THE FIRST INPUT IS ENTERED FOLLOWED BY THE OPERATION PERFORM THE8
    }
    ...
    //create a override method for changing the last operand and click another operand button
    private void changeLastOperand(String input, String operand)
    {
        String lastOfExpression = input.charAt(input.length() - 1) + "";
        if (input.length() > 1)
        {
            if (lastOfExpression.equals(")"))
            {
                ...
                for (int i = input.length() - 2; i >= 0; i--)
                {
                    ...
                    {
                        lastExpression = operand + lastExpression;
                        break;
                    } else
                    {
                        lastExpression = "";
